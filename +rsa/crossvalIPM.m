function [G,Sig]=crossvalIPM(B,partition,conditionVec,X)
% function [G,Sigma]=crossvalIPM(Y,partition,conditionVec,X);
% Calculates the crossvalidated inner product matrix (IPM) between activation patterns 
% (estimated beta weights) from the first level analysis. The IPM is calculated 
...
% means that the estimates can be negative (see Walther et al., 2015).  
