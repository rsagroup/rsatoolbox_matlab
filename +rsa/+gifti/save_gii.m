function rsa_save_gii(this,filename,encoding)
...
% Modified:
% 2/15 Naveed Ejaz
%   - Data is now written as RowMajorOrder as opposed to ColumnMajorOrder,
%   the later is incompatible with Caret
%
...
%     this.data{i}.attributes.ArrayIndexingOrder = 'ColumnMajorOrder';
    this.data{i}.attributes.ArrayIndexingOrder = 'RowMajorOrder';
...
%             sprintf('%f ',this.data{i}.space(j).MatrixData));
        fprintf(fid,'%s<MatrixData>\n%s</MatrixData>\n',o(3),...
            sprintf([repmat('%f ', 1, size(this.data{i}.space(j).MatrixData, 2)) '\n'],...
            this.data{i}.space(j).MatrixData'));
        fprintf(fid,'%s</CoordinateSystemTransformMatrix>\n',o(2));
    end
    ...
    switch this.data{i}.attributes.Encoding
        case 'ASCII'
            dataTemp = this.data{i}.data';
            fprintf(fid, [tp.format ' '], dataTemp);
%             fprintf(fid,[repmat([tp.format ' '], 1, size(this.data{i}.data, 2)) '\n'], this.data{i}.data');
        case 'Base64Binary'
%             fprintf(fid,base64encode(typecast(this.data{i}.data(:),'uint8')));
            dataTemp = this.data{i}.data';
            fprintf(fid,base64encode(typecast(dataTemp(:),'uint8')));
            % uses native machine format
        case 'GZipBase64Binary'
%             fprintf(fid,base64encode(dzip(typecast(this.data{i}.data(:),'uint8'))));
            dataTemp = this.data{i}.data';
            fprintf(fid,base64encode(dzip(typecast(dataTemp(:),'uint8'))));
            % uses native machine format
        case 'ExternalFileBinary'
            extfilename = this.data{i}.attributes.ExternalFileName;
